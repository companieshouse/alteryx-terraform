...
# ------------------------------------------------------------------------------
# Worker Security Group and rules
# ------------------------------------------------------------------------------
...
resource "aws_vpc_security_group_ingress_rule" "ec2_worker_wmi_azure" {
  for_each = toset(local.azure_dc_cidrs)
...
resource "aws_vpc_security_group_ingress_rule" "ec2_worker_ansible" {
  description       = "Ansible Access"
  security_group_id = aws_security_group.ec2_worker.id
  prefix_list_id    = data.aws_ec2_managed_prefix_list.ci.id
  ip_protocol       = "tcp"
...
resource "aws_vpc_security_group_egress_rule" "ec2_worker_egress" {
  security_group_id = aws_security_group.ec2_worker.id
